...
def Generator(z_g, z_l, labels):
    ...
    output = lib.ops.deconv1d.Deconv1D('Generator.2', DIM_LATENT_G, 4*DIM, 5, output,cond=cond)
    if BN_FLAG_G:
...
al=list(zip(t_data,tt_y))
np.random.shuffle(al)
t_data= [x[0] for x in al]
tt_y=np.array([x[1] for x in al])
tt_y = binarize_labels(tt_y)

s_data,ss_y = dev_gen().next()

ss_y = binarize_labels(ss_y)

def reconstruct_video(iteration):
    rec_samples = session.run(rec_x, feed_dict={real_x_unit: fixed_data, real_y:fixed_y, t_x:t_data,t_y:tt_y,same_x:s_data,same_y:ss_y})
